(() => {
    ...
    function rangeInit() {
        ...
        if (rangeSlider.length) rangeSlider.forEach((range => {
        ...
        if (rangeOneSlider.length) rangeOneSlider.forEach((range => {
            let minValue = parseInt(range.getAttribute("data-min"), 10);
            let maxValue = parseInt(range.getAttribute("data-max"), 10);
            initialize(range, {
                start: maxValue,
            ...
            if (nonLinearStepSliderValueElement) range.noUiSlider.on("update", (function(values) {
    ...
    function initSliders() {
        ...
        if (document.querySelectorAll(".review__slider--left")) new Swiper(".review__slider--left", {
            modules: [ Autoplay ],
            observer: true,
            observeParents: true,
            slidesPerView: 4,
        ...
        if (document.querySelectorAll(".review__slider--right")) new Swiper(".review__slider--right", {
            modules: [ Autoplay ],
            observer: true,
            observeParents: true,
            slidesPerView: 4,
    ...
    addActiveClass(".nav-select__current", "open-select");
    addActiveClass(".services-links__current", "open-services-links");
    document.addEventListener("click", (e => {
        ...
        if (!targetElement.closest(".header__support-btn")) document.documentElement.classList.remove("open-support");
        if (targetElement.closest(".top-nav__user")) targetElement.closest(".top-nav__profile").classList.add("active-user");
        if (targetElement.closest(".calc-aside__code-btn")) targetElement.closest(".calc-aside__code-body").classList.add("active");
        if (targetElement.closest(".calc-aside__code-reset")) targetElement.closest(".calc-aside__code-body").classList.remove("active");
    }));
    const selectInit = () => {
        ...
        if (selectArray.length > 0) {
            selectArray.forEach(((element, index) => {
                ...
                const selectWrapper = document.createElement("div"), selectLabel = document.createElement("div"), selectItemList = document.createElement("div"), selectLabelSpan = document.createElement("span"), selectLabelArea = document.createElement("span");
                ...
                if (selectTag[selectTag.selectedIndex].dataset.icon) element.style.setProperty("--bg-name", `url('${selectTag[selectTag.selectedIndex].dataset.icon}')`);
                ...
                selectOption.forEach(((option, index) => {
                    ...
                    selectItem.addEventListener("click", (e => {
                        const selectItemTag = e.target.closest(".custom-select").querySelector("select"), selectItemOptions = selectItemTag.querySelectorAll("option");
                ...
                selectLabelArea.addEventListener("click", (e => {
                    ...
                    window.innerHeight - selectItemList.getBoundingClientRect().bottom < 100 ? (selectItemList.classList.add("select__content--top"), 
                    selectLabel.classList.add("select__label--top")) : (selectItemList.classList.remove("select__content--top"), 
                ...
                selectTag.addEventListener("change", (() => {
                    const selectItemOptions = selectTag.options, selectedItem = selectItemOptions[selectTag.selectedIndex];
                    selectLabelSpan.textContent = selectedItem.textContent;
                    element.style.setProperty("--bg-name", `url('${selectedItem.dataset.icon}')`);
                    const allItems = selectItemList.querySelectorAll(".select__item");
                    allItems.forEach(((item, idx) => {
                        if (idx === selectTag.selectedIndex) item.classList.add("selected"); else item.classList.remove("selected");
                    }));
                }));
            }));
    ...
    const closeAllSelect = select => {
        const selectContentArray = document.querySelectorAll(".select__content"), selectLabelArray = document.querySelectorAll(".select__label");
        const customSelect = document.querySelectorAll(".custom-select");
        selectLabelArray.forEach(((element, index) => {
            element !== select ? (element.classList.remove("select__label--active"), selectContentArray[index].classList.add("select__content--hidden"), 
            customSelect[index].classList.remove("active")) : null;
        }));
    };
    selectInit();
    menuInit();
